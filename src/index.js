...
async function main() {
    ...
    const projectName = await text({
        ...
        validate: (input) => (input ? undefined : 'Project name cannot be empty'),
    });
    ...
    const importAlias = await text({
        ...
        placeholder: '~/',
        initialValue: '~/',
        validate: (input) => (input ? undefined : 'Import alias cannot be empty'),
    });
    if (isCancel(importAlias))
        return cancel('Operation cancelled.');
    ...
    if (databaseProvider)
        console.log(`  Database:     ${chalk.cyan(databaseProvider)}`);
